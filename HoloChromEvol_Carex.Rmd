...
fitHoloChromEvol=function(states){
  ...
  # define set of models to compare
	...
	for(m in 1:length(models)){
		cat("\n\n\n\n\t*** ", paste(toupper(summaries[m]),": fitting ", sep=""), models[m],
			 " *** \n", sep="")
		tmp=fitContinuous(phy,states,SE=0,model=models[m], ncores=2)
		print(tmp)
		aic[m]=tmp$opt$aicc
		lnl[m]=tmp$opt$lnL
	}
...
mu.d.h.s <- sigmoid.x(states.seq, chrom.mle.d.h.s$par[5], chrom.mle.d.h.s$par[6], chrom.mle.d.h.s$par[7], chrom.mle.d.h.s$par[8])
plot(states.seq, mu.d.h.s, type = "l")

lambda.d.h.s <- noroptimal.x(states.seq, chrom.mle.d.h.s$par[1], chrom.mle.d.h.s$par[2], chrom.mle.d.h.s$par[3], chrom.mle.d.h.s$par[4])
plot(states.seq, lambda.d.h.s, type = "l")

plot(states.seq, mu.d.h.s, type = "l")
lines(states.seq, lambda.d.h.s, type = "l")
